...
- [x] Vercel 再デプロイ（现公開URLの動作確認）
- [x] Vercelログで Deploying outputs の結果と静的成果物の準備状況を確認
- [x] `GET /` が 200、`/api/tasks` が 200 になるか確認

- [x] ブラッシュアップ要件計画確定（Supabase Auth + reminders + task_links + RLS）
- [x] （実装）Supabase導入（環境変数/接続/認証）

- [ ] （実装）DBスキーマ移行（tasks/task_links/push_subscriptions + RLS）
- [x] （実装）APIガード（認証必須 + 自分のデータだけアクセス）

- [x] （実装）フロント拡張（remind_at入力/UI）
